...
class AroWallet {
  constructor(key) {
    if (isString(key)) {
      this.keypair = this.decode(key);
    } else {
      this.keypair = key ? key : ec.genKeyPair();
    }
    var {pem, publicCoin, privateCoin, encoded} = encodeKeypair(this.keypair);

		this.pem = pem;
    this.publicKey = publicCoin;
    this.privateKey = privateCoin;
