...
test('Wallet can export an unencrypted ARO format', t => {
	const w = new AroWallet();
	const aro = w.getWalletFormat();
	t.truthy(aro);
...
test('Encrypted Wallet gets encrypted aro', t => {
	const w = new AroWallet(null, 'test');
	const aro = w.getWalletFormat('test');
	t.truthy(aro);
